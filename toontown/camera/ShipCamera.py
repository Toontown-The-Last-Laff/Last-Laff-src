...
from toontown.camera.OrbitCamera import OrbitCamera
import math
...
class ShipCamera(OrbitCamera):
    ...
    def _mouseUpdateTask(self, task):
        if OrbitCamera._mouseUpdateTask(self, task) == task.cont and self.mouseDelta[0] or self.mouseDelta[1]:
            ...
            self.camTimer = -1.0
            self.setRotation(self.getRotation() - self.mouseDelta[0] * sensitivity[0])
            self.setEscapement(clampScalar(self.escapement + self.mouseDelta[1] * sensitivity[1] * 0.6, self._minEsc, self._maxEsc))
        return task.cont
