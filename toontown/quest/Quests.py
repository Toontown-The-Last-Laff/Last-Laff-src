...
class CogLevelTypeQuest(CogQuest):
    ...
    def doesCogCount(self, avId, cogDict, zoneId, avList):
        questCogType=self.getCogType()
        questCogLevel=self.getCogLevel()
        return questCogType is Any or questCogType is cogDict['type'] and questCogLevel <= cogDict['level'] and avId in \
               cogDict['activeToons'] and self.isLocationMatch(zoneId)
